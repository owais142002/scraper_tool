...
db={}
def ghanaPropertyCentreDetails():
    read_db()
    ...
    db['ghanapropertycentre.com'][0]=date
    db['ghanapropertycentre.com'][1]=timeStarted
    db['ghanapropertycentre.com'][2]='-'
    db['ghanapropertycentre.com'][3]='-'
    db['ghanapropertycentre.com'][-1]='running'    
    write_db()
    ...
    def getUrls():
        urls = [f"https://ghanapropertycentre.com/for-rent/short-let?page={i}" for i in range(1, 15)]
        urls += [f"https://ghanapropertycentre.com/for-rent?page={i}" for i in range(1, 120)]
        urls += [f"https://ghanapropertycentre.com/for-sale?page={i}" for i in range(1, 200)]


        def extract_links(url):
            response = requests.get(url, timeout=120)
            soup = BeautifulSoup(response.text, 'lxml')
        #     if not soup.select_one('.cards-container > div:nth-of-type(2)'):
        #         print('here')
        #         return
#             print(url)
...
#                 print(link[0], propertyTitle, propertyId)
    ...
    try:
        ...
        db['ghanapropertycentre.com'][2]=timeEnded
        db['ghanapropertycentre.com'][3]=dateEnded
        db['ghanapropertycentre.com'][-1]='completed'
        write_db()
    except Exception as e:
        db['ghanapropertycentre.com'][-1]=f'error occured-->{e}'
        write_db()
